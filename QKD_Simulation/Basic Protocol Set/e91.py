...
class QuantumChannel:
    ...
    def transmit(self, qubit_state):
        ...
        # Apply noise moved to simulation
        return qubit_state
    ...
    def generate_random_bases(self, n):
        ...
        for _ in range(n):
            if random.random() < 0.7:  # 70% for key generation
                self.bases.append(random.randint(0, 1))  # 0 or 1
...
class E91Simulation:
    ...
    def __init__(self, distance_km=10, initial_pairs=1000):
        ...
        # Added for correct simulation
        self.alice_thetas = [0, 22.5, 45]
        self.bob_thetas = [-22.5, 0, 22.5]
        self.detector_efficiency = 0.85
        self.measurement_error_rate = 0.01 + (0.0005 * distance_km)
    ...
    def _entanglement_distribution_phase(self):
        ...
        for i in range(self.initial_pairs):
            ...
            if alice_photon is not None and bob_photon is not None:
                basis_a = self.alice.bases[i]
                basis_b = self.bob.bases[i]
                theta_a = self.alice_thetas[basis_a]
                theta_b = self.bob_thetas[basis_b]
                delta = theta_a - theta_b
                if random.random() < self.ent_source.entanglement_fidelity:
                    E = math.cos(2 * math.radians(delta))
                else:
                    E = 0
                prob_same = (1 + E) / 2
                alice_result = random.randint(0, 1)
                bob_result = alice_result if random.random() < prob_same else 1 - alice_result
                alice_result = self.noise_model.apply_noise(alice_result)
                bob_result = self.noise_model.apply_noise(bob_result)
                if random.random() > self.detector_efficiency:
                    alice_result = None
                if random.random() > self.detector_efficiency:
                    bob_result = None
                if alice_result is not None and random.random() < self.measurement_error_rate:
                    alice_result = 1 - alice_result
                if bob_result is not None and random.random() < self.measurement_error_rate:
    ...
    def _sifting_phase(self):
        ...
        for measurement in self.successful_measurements:
            ...
            # Key generation: use measurements where angles match
            if self.alice_thetas[alice_basis] == self.bob_thetas[bob_basis]:
    ...
    def _bell_inequality_test(self):
        ...
        # CHSH inequality calculation with correct combinations
        E_A0B0 = E.get((0,0), 0)
        E_A0B1 = E.get((0,2), 0)
        E_A1B0 = E.get((2,0), 0)
        E_A1B1 = E.get((2,2), 0)
        S = abs(E_A0B0 + E_A0B1 + E_A1B1 - E_A1B0)
...
def main():
    ...
    simulation = E91Simulation(distance_km=10, initial_pairs=1000)  # Change to 10000 for e91.py
